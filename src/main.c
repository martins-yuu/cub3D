...
/*   Updated: 2024/07/15 14:08:25 by yuuko            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_background(void *param)
{
	...
	bg = param;
	ctx_image_resize(bg, ctx()->width, ctx()->height);
	i = 0;
...
void	handle_movement(void *param)
{
	...
	img = param;
	if (mlx_is_key_down(ctx(), MLX_KEY_ESCAPE))
		mlx_close_window(ctx());
	if (mlx_is_key_down(ctx(), MLX_KEY_K))
		img->instances[0].y -= 10;
	if (mlx_is_key_down(ctx(), MLX_KEY_J))
		img->instances[0].y += 10;
	if (mlx_is_key_down(ctx(), MLX_KEY_H))
		img->instances[0].x -= 10;
	if (mlx_is_key_down(ctx(), MLX_KEY_L))
		img->instances[0].x += 10;
}
